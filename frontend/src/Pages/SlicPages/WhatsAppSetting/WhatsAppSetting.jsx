...
const WhatsAppSetting = () => {
  ...
  const fetchWhatsAppData = async () => {
    ...
    try {
        ...
        // Silently handle error if not connected - user can manually connect
        console.log("WhatsApp not connected yet:", err?.response?.data?.message);
        // Only show error toast if it's an unexpected error, not just "not connected"
        if (err?.response?.data?.needsConnection) {
          console.log("WhatsApp client needs connection");
  ...
  // Give backend time to auto-initialize with existing session
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      fetchWhatsAppData();
    }, 8000); // Increased from 5s to 8s to allow backend more time to initialize
 ...
 const logOutSession = async () => {
    ...
    //   console.log(data);
      ...
      // Clear profile data after logout
      setUserName('');
      setMobileNumber('');
  ...
  const handleClosePopup = () => {
    ...
    // Wait longer after QR scan for client to fully initialize and become ready
    setTimeout(() => {
      fetchWhatsAppData();
    }, 15000) // Increased from 5s to 15s to allow client to fully initialize
  };
